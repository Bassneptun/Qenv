...
TEST(QUBIT, HADDAMARD) {
  ...
  q.haddamard();
  EXPECT_EQ(q.measure(), 0) << vector_to_string(to_vector(q.get()));
}
...
TEST(QUBITD, SWAP) {
  ...
  EXPECT_TRUE(q1.swap(q2).get().n_elem == 4);
  EXPECT_TRUE(vectors_are_close(to_vector(q1.swap(q2).get()),
                                to_vector(comp.get()), TOLERANCE))
      << vector_to_string(to_vector(q1.swap(q2).get()));
}
...
TEST(QUBITF, RX) {
  ...
  q.set({1 / sqrt(2), 1 / sqrt(2)});
  EXPECT_TRUE(
      vectors_are_close(to_vector(q.rx(1).get()), to_vector(comp.get()), 0.01))
      << vector_to_string(to_vector(q.rx(1.).get())) << "\n";
}
...
TEST(QUBITF, RY) {
  ...
  q.set({1 / sqrt(2), 1 / sqrt(2)});
  EXPECT_TRUE(q.ry(1).get().n_elem == 2);
  q.set({1 / sqrt(2), 1 / sqrt(2)});
  EXPECT_TRUE(vectors_are_close(to_vector(q.ry(1).get()), to_vector(comp.get()),
                                TOLERANCE))
      << vector_to_string(to_vector(q.ry(1).get()));
}
...
TEST(QUBITF, RZ) {
  ...
  q.set({1 / sqrt(2), 1 / sqrt(2)});
  EXPECT_TRUE(q.rz(1).get().n_elem == 2);
  q.set({1 / sqrt(2), 1 / sqrt(2)});
